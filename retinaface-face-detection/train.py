...
from modules.utils import (set_memory_growth, load_yaml, load_dataset, ProgressBar)


flags.DEFINE_string('cfg_path', 'lightcnn-face-recognition\config.yaml','config file path')
flags.DEFINE_string('gpu', '0', 'which gpu to use')
flags.DEFINE_string('output', 'lightcnn-face-recognition\model', 'which gpu to use')
...
def main(_):
    ...
    # load checkpoint
    checkpoint_dir = FLAGS.output + '/checkpoints/' + cfg['sub_name']
    ...
    # training loop
    summary_writer = tf.summary.create_file_writer(FLAGS.output + '/logs/' + cfg['sub_name'])
    remain_steps = max(
